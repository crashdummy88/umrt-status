...
- Xbox-style overall banner + interactive network list + MT·WY·ID·WA corridor tiles + OR·CA·NM·NV·AZ winter expansion + Cloudflare strip
...
| `headline` | Line under “Location / corridor” — keep both active corridor and winter expansion in this sentence |
| `locations[].state` | Active corridor (MT WY ID WA): `open` · `limited` · `closed` · `seasonal` |
| `winterExpansion.locations[].state` | OR CA NM NV AZ — same four values. `seasonal` shows as **Coming soon** |
| `winterExpansion.kicker` | Group line under “Winter expansion” |
| `winterExpansion.dates` | Optional. Only add real dates here; otherwise the board stays “no service dates posted yet” |
| `beyond` | Optional off-corridor tile (`MI / WI / SD · MN / ND`) |
